...
class AssetContentRepository implements ContentRepository {
  ...
  Future<void> _ensureLoaded() async {
    ...
    for (final file in files) {
      ...
      final sectionPayloads = decoded['sections'] as List?;
      if (sectionPayloads != null) {
        for (final raw in sectionPayloads.cast<Map<String, dynamic>>()) {
          final section = CampaignSection(
            id: raw['id'] as String,
            testament: Testament.fromKey(raw['testament'] as String),
            title: raw['title'] as String,
            description: raw['description'] as String? ?? '',
            orderIndex: raw['orderIndex'] as int? ?? raw['order_index'] as int? ?? 0,
            requiredLevels: raw['requiredLevels'] as int? ?? raw['required_levels'] as int? ?? 0,
            iconKey: raw['iconKey'] as String? ?? raw['icon_key'] as String? ?? 'book',
          );
